...
# Creating VPC
resource "aws_vpc" "Project-Anon-vpc" {
  cidr_block = "14.0.0.0/16"
  instance_tenancy = "default"
  tags = {
    Name = "Project-Anon-VPC"
  }
}

# Creating 2 Subnets 
resource "aws_subnet" "Project-pub-sub-a1" {
  vpc_id     = aws_vpc.Project-Anon-vpc.id
...
resource "aws_instance" "Ansible-Master" {
  ...
  provisioner "remote-exec" {
    connection {
      type     = "ssh"
      user     = "ubuntu"
      private_key = file("/path/to/Anon.pem")
      host     = self.public_ip
    }
    inline = [
      "sudo -i apt update -y",
...
resource "null_resource" "Copy_Yaml_Files" {
  provisioner "local-exec" {
    command= "scp -o StrictHostKeyChecking=no -i /path/to/Anon.pem /path/to/*.yaml ubuntu@${aws_instance.Ansible-Master.public_ip}:/home/ubuntu/"
  }
  provisioner "remote-exec" {
    connection {
      type     = "ssh"
